...
async function toggleLike(postId) {
  try {
    ...
    // Check if user already liked this post using maybeSingle (No 406 Error)
    const { data: existingLike, error: fetchError } = await sb
      .from('post_likes')
      .select('id')
      .eq('post_id', postIdNum)
      .eq('user_id', currentUser.id)
      .maybeSingle(); // ✅ FIXED: Uses maybeSingle() instead of single()
    ...
    if (existingLike) {
      ...
      if (error) {
        // If table doesn't exist, create it (Feature from complete file)
...
async function getUserLiked(postId) {
  try {
    ...
    const { data, error } = await sb
      ...
      .eq('post_id', postIdNum)
      .eq('user_id', currentUser.id)
      .maybeSingle(); // ✅ FIXED: Uses maybeSingle()
